...
namespace AcmMvcApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (!isUserInRole(User.Identity.Name, "ADMIN")) return RedirectToAction("PageUnavailable");
        ...
        public static bool isUserInRole(string userLogin, string role)
        {
        ...
        //
        // GET: /Account/PageUnavailable

        [AllowAnonymous]
        public ActionResult PageUnavailable()
        {
            return View("PageUnavailable");
        }
        ...
        {
            if (ModelState.IsValid && isUserActive(model.UserName))
            {
                if (WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
                {
                    isUserAdmin = Roles.IsUserInRole(model.UserName, "ADMIN");
        ...
        {
            foreach (UserProfile up in db.UserProfiles.ToArray())
            {
                if (up.UserName.Equals(userName) && up.IsActive) return true;
            }
        ...
        public ActionResult Create()
        {
            ...
            regModel.selectedRole = "MANAGER";
            regModel.isActive = true;
        ...
        {
            UserProfileEditModel editUserModel = new UserProfileEditModel();

            editUserModel.userProfile = db.UserProfiles.Find(id);
            editUserModel.selectedRole = Roles.IsUserInRole(editUserModel.userProfile.UserName, "ADMIN") ? "ADMIN" : "MANAGER";
        ...
        [HttpPost]
        public ActionResult Edit(UserProfileEditModel editUserModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (editUserModel.NewPassword != null)
                {
                    WebSecurity.ResetPassword(WebSecurity.GeneratePasswordResetToken(editUserModel.userProfile.UserName), editUserModel.NewPassword);
                }

                string[] allRoles = Roles.GetRolesForUser(editUserModel.userProfile.UserName);
                if (allRoles != null && allRoles.Length > 0)
                    Roles.RemoveUserFromRoles(editUserModel.userProfile.UserName, Roles.GetRolesForUser(editUserModel.userProfile.UserName));

                Roles.AddUserToRole(editUserModel.userProfile.UserName, editUserModel.selectedRole);

                db.Entry(editUserModel.userProfile).State = EntityState.Modified;
                db.SaveChanges();
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult RegisterByAdmin(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                // Попытка зарегистрировать пользователя
